...
//Connection Settings @0-3C7FE332
$CCConnectionSettings = array (
    "mysql_cams_v2" => array(
        "Type" => "MySQL",
        "DBLib" => "MySQL",
        "Database" => "cams_ffsl_v2",
        "Host" => "localhost",
        "Port" => "3306;Database=cams_ffsl_v2",
        "User" => "root",
        "Password" => "",
...
//Initialize Common Variables @0-8801F28E
...
$CCSDesign = "";
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", 'AoN3e870h5b7evd1');
define("CCS_EXPIRATION_DATE", 30 * 24 * 3600);
define("CCS_SLIDING_EXPIRATION", false);

$DefaultDateFormat = array("yyyy", "-", "mm", "-", "dd");

$MainPage = new clsMainPage();
//End Initialize Common Variables

//mysql_cams_v2 Connection Class @-C949926F
class clsDBmysql_cams_v2 extends DB_Adapter
{
    function clsDBmysql_cams_v2()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["mysql_cams_v2"]);
        parent::Initialize();
...
//End mysql_cams_v2 Connection Class
...
function CCSetALCookie($login, $password) {
    ...
    CCSetCookie("cams_ffsl_v2Login", $result, time() + CCS_EXPIRATION_DATE, "/", "", false);
}
//End CCSetALCookie

//CCRefreshALCookie @0-2DB71C09
function CCRefreshALCookie($expirationDate) {
    if (CCS_SLIDING_EXPIRATION) {
        if (($expirationDate - (CCS_EXPIRATION_DATE / 2)) > time()) {
            list($login, $password, $expDate) = CCParseALCookie("cams_ffsl_v2Login");
...
//CCCheckAutoLoginCookies @0-37A9E118
function CCCheckAutoLoginCookies() {
    if (!CCGetUserLogin()) {
        $parts = CCParseALCookie("cams_ffsl_v2Login");
...
//CCLoginUser @0-F6E7930C
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBmysql_cams_v2();
    $SQL = "SELECT mfi_emp_name, mfi_emp_user_type, mfi_emp_user_pass FROM mfi_emps WHERE mfi_emp_user_name=" . $db->ToSQL($login, ccsText) . " AND mfi_emp_user_pass=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("mfi_emp_name"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("mfi_emp_user_type"));
