<?php
//Include Common Files @1-A315AAD5
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "BatchDataEntry.php");
...
//Class_Initialize Event @29-13E8DCC5
    function clsRecordmfi_docs($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->mfi_doc_updatedon = new clsControl(ccsHidden, "mfi_doc_updatedon", "mfi_doc_updatedon", ccsDate, array("yyyy", "-", "mm", "-", "dd", " ", "H", ":", "nn", ":", "ss"), CCGetRequestParam("mfi_doc_updatedon", $Method, NULL), $this);
            ...
            $this->ImageLink1->Page = "ChangeRegionBatch.php";
            $this->doc_rejection_reason = new clsControl(ccsListBox, "doc_rejection_reason", "doc_rejection_reason", ccsText, "", CCGetRequestParam("doc_rejection_reason", $Method, NULL), $this);
            $this->doc_rejection_reason->DSType = dsListOfValues;
            $this->doc_rejection_reason->Values = array(array("DUPLICATE LOGIN", "DUPLICATE LOGIN"), array("POOR QUALITY IMAGE", "POOR QUALITY IMAGE"));
            if(!$this->FormSubmitted) {
                if(!is_array($this->mfi_doc_status->Value) && !strlen($this->mfi_doc_status->Value) && $this->mfi_doc_status->Value !== false)
                    $this->mfi_doc_status->SetText("DATA ENTERED");
                if(!is_array($this->mfi_doc_updatedon->Value) && !strlen($this->mfi_doc_updatedon->Value) && $this->mfi_doc_updatedon->Value !== false)
                    $this->mfi_doc_updatedon->SetValue(time());
            }
        }
    }
//End Class_Initialize Event

//Initialize Method @29-E7DAD703
    function Initialize()
    {
...
//Validate Method @29-0EBCB9FC
    function Validate()
    {
        ...
        $Validation = ($this->doc_rejection_reason->Validate() && $Validation);
        ...
        $Validation =  $Validation && ($this->doc_rejection_reason->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @29-49ECC779
...
//UpdateRow Method @29-D14BED4C
    function UpdateRow()
    {
        ...
        $this->DataSource->mfi_doc_id->SetValue($this->mfi_doc_id->GetValue(true));
        $this->DataSource->doc_rejection_reason->SetValue($this->doc_rejection_reason->GetValue(true));
        $this->DataSource->mfi_doc_updatedon->SetValue($this->mfi_doc_updatedon->GetValue(true));
...
//Show Method @29-6F2C7A13
    function Show()
    {
        ...
        $this->doc_rejection_reason->Prepare();
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->doc_rejection_reason->Errors->ToString());
...
//DataSource Variables @29-AA131376
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @29-441016C8
    function clsmfi_docsDataSource(& $Parent)
    {
        ...
        $this->mfi_doc_updatedon = new clsField("mfi_doc_updatedon", ccsDate, $this->DateFormat);
        ...
        $this->UpdateFields["doc_rejection_reason"] = array("Name" => "doc_rejection_reason", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["mfi_doc_entered_at"] = array("Name" => "mfi_doc_entered_at", "Value" => "", "DataType" => ccsDate, "OmitIfEmpty" => 1);
    }
//End DataSourceClass_Initialize Event

//Prepare Method @29-28194273
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->AddParameter("3", "cookbatch_code", ccsText, "", "", $this->Parameters["cookbatch_code"], "", false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
        $this->wp->Criterion[1] = "( mfi_doc_status='NUMBERED' and numbering_errors like 'MATCH' )";
        ...
        $this->wp->Criterion[3] = $this->wp->Operation(opEqual, "batch_code", $this->wp->GetDBValue("3"), $this->ToSQL($this->wp->GetDBValue("3"), ccsText),false);
...
//SetValues Method @29-59913A9E
    function SetValues()
    {
        ...
        $this->mfi_doc_updatedon->SetDBValue(trim($this->f("mfi_doc_updatedon")));
        $this->doc_rejection_reason->SetDBValue($this->f("doc_rejection_reason"));
    }
//End SetValues Method

//Update Method @29-89EE66A3
    function Update()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->cp["doc_rejection_reason"] = new clsSQLParameter("ctrldoc_rejection_reason", ccsText, "", "", $this->doc_rejection_reason->GetValue(true), NULL, false, $this->ErrorBlock);
        $this->cp["mfi_doc_entered_at"] = new clsSQLParameter("ctrlmfi_doc_updatedon", ccsDate, $DefaultDateFormat, array("yyyy", "-", "mm", "-", "dd", " ", "HH", ":", "nn", ":", "ss"), $this->mfi_doc_updatedon->GetValue(true), NULL, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["doc_rejection_reason"]->GetValue()) and !strlen($this->cp["doc_rejection_reason"]->GetText()) and !is_bool($this->cp["doc_rejection_reason"]->GetValue())) 
            $this->cp["doc_rejection_reason"]->SetValue($this->doc_rejection_reason->GetValue(true));
        if (!is_null($this->cp["mfi_doc_entered_at"]->GetValue()) and !strlen($this->cp["mfi_doc_entered_at"]->GetText()) and !is_bool($this->cp["mfi_doc_entered_at"]->GetValue())) 
            $this->cp["mfi_doc_entered_at"]->SetValue($this->mfi_doc_updatedon->GetValue(true));
        ...
        $this->UpdateFields["doc_rejection_reason"]["Value"] = $this->cp["doc_rejection_reason"]->GetDBValue(true);
        $this->UpdateFields["mfi_doc_entered_at"]["Value"] = $this->cp["mfi_doc_entered_at"]->GetDBValue(true);
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "BatchDataEntry.html";
$BlockToParse = "main";
...
//Include events file @1-E30CB4EE
include_once("./BatchDataEntry_events.php");
...
$ListBox3->Values = array(array("CP", "Centre Proposal"), array("KYC", "KYC"), array("LA1", "LA FORM1"), array("LA2", "LA FORM 2"), array("INVALID IMAGE", "INVALID IMAGE"));
