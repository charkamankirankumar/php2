<?php
//Include Common Files @1-32ED9E21
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "doc_Numbering.php");
...
//Class_Initialize Event @128-67A3090D
...
"FROM mfi_unit_regions {SQL_Where} {SQL_OrderBy}";
            ...
            if(!$this->FormSubmitted) {
                if(!is_array($this->mfi_doc_status->Value) && !strlen($this->mfi_doc_status->Value) && $this->mfi_doc_status->Value !== false)
                    $this->mfi_doc_status->SetText("NUMBERED");
            }
...
//CheckErrors Method @128-FCA0279D
...
//DataSource Variables @128-1820570F
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @128-DECBDAF3
    function clsmfi_docsDataSource(& $Parent)
    {
        ...
        $this->UpdateFields["numbered_at"] = array("Name" => "numbered_at", "Value" => "", "DataType" => ccsDate, "OmitIfEmpty" => 1);
    }
//End DataSourceClass_Initialize Event

//Prepare Method @128-A0BA476B
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->Criterion[2] = "( mfi_doc_status='PRENUMBERED' )";
...
//Update Method @128-7571EACA
    function Update()
    {
        ...
        $Where = "";
        $this->cp["mfi_doc_territory_code"] = new clsSQLParameter("ctrlListBox3", ccsText, "", "", $this->ListBox3->GetValue(true), NULL, false, $this->ErrorBlock);
        $this->cp["mfi_doc_status"] = new clsSQLParameter("expr163", ccsText, "", "", "NUMBERED", NULL, false, $this->ErrorBlock);
        $this->cp["numbered_by"] = new clsSQLParameter("ctrlmfi_doc_tagged_by", ccsText, "", "", $this->mfi_doc_tagged_by->GetValue(true), NULL, false, $this->ErrorBlock);
        ...
        $this->cp["numbered_at"] = new clsSQLParameter("ctrlmfi_doc_tagged_at", ccsDate, $DefaultDateFormat, $this->DateFormat, $this->mfi_doc_tagged_at->GetValue(true), NULL, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["mfi_doc_territory_code"]->GetValue()) and !strlen($this->cp["mfi_doc_territory_code"]->GetText()) and !is_bool($this->cp["mfi_doc_territory_code"]->GetValue())) 
            $this->cp["mfi_doc_territory_code"]->SetValue($this->ListBox3->GetValue(true));
        if (!is_null($this->cp["mfi_doc_status"]->GetValue()) and !strlen($this->cp["mfi_doc_status"]->GetText()) and !is_bool($this->cp["mfi_doc_status"]->GetValue())) 
            $this->cp["mfi_doc_status"]->SetValue("NUMBERED");
        if (!is_null($this->cp["numbered_by"]->GetValue()) and !strlen($this->cp["numbered_by"]->GetText()) and !is_bool($this->cp["numbered_by"]->GetValue())) 
            $this->cp["numbered_by"]->SetValue($this->mfi_doc_tagged_by->GetValue(true));
        ...
        if (!is_null($this->cp["numbered_at"]->GetValue()) and !strlen($this->cp["numbered_at"]->GetText()) and !is_bool($this->cp["numbered_at"]->GetValue())) 
            $this->cp["numbered_at"]->SetValue($this->mfi_doc_tagged_at->GetValue(true));
        ...
        $this->UpdateFields["numbered_by"]["Value"] = $this->cp["numbered_by"]->GetDBValue(true);
        ...
        $this->UpdateFields["numbered_at"]["Value"] = $this->cp["numbered_at"]->GetDBValue(true);
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "doc_Numbering.html";
$BlockToParse = "main";
...
//Include events file @1-53B543A5
include_once("./doc_Numbering_events.php");
