...
//End Custom Code
 ...
 // -------------------------
  ...
  $conn = new clsDBmysql_cams_v2();
   $query = "UPDATE mfi_docs SET mfi_doc_status = 'PRENUMBERING', mfi_doc_updatedon = NOW()  WHERE mfi_doc_id =".CCGetParam(mfi_doc_id)." ;";
   ...
   $query = "UPDATE mfi_docs SET mfi_doc_status = 'TAGGED', mfi_doc_updatedon = NOW()  WHERE mfi_doc_updatedon < DATE_SUB(NOW(),INTERVAL 4 MINUTE) AND mfi_doc_status = 'PRENUMBERING' AND isnull(pre_numbered_at)";
   $result = $conn->query($query);
 // -------------------------
