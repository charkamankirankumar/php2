...
//End Custom Code
 ...
 // -------------------------
  ...
  $conn = new clsDBmysql_cams_v2();
   $query = "UPDATE mfi_docs SET mfi_doc_status = 'DEDUPING', mfi_doc_updatedon = NOW()  WHERE mfi_doc_id =".$_POST["mfi_doc_id"]." ;";
   ...
   $query = "UPDATE mfi_docs SET mfi_doc_status = 'DATA ENTERED', mfi_doc_updatedon = NOW()  WHERE mfi_doc_updatedon < DATE_SUB(NOW(),INTERVAL 5 MINUTE) AND mfi_doc_status = 'DATA ENTRY' AND isnull(mfi_doc_entered_at)";
   $result = $conn->query($query);
 // -------------------------
