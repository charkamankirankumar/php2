<?php
//Include Common Files @1-4BBB5681
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "doc_Pre_Numbering.php");
...
//Class_Initialize Event @128-71708745
    function clsRecordmfi_docs($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->ListBox3 = new clsControl(ccsTextBox, "ListBox3", "ListBox3", ccsText, "", CCGetRequestParam("ListBox3", $Method, NULL), $this);
...
"FROM mfi_unit_regions {SQL_Where} {SQL_OrderBy}";
            ...
            $this->pre_numbered_by = new clsControl(ccsHidden, "pre_numbered_by", "pre_numbered_by", ccsText, "", CCGetRequestParam("pre_numbered_by", $Method, NULL), $this);
            if(!$this->FormSubmitted) {
                if(!is_array($this->mfi_doc_status->Value) && !strlen($this->mfi_doc_status->Value) && $this->mfi_doc_status->Value !== false)
                    $this->mfi_doc_status->SetText("PRENUMBERED");
            }
...
//Initialize Method @128-F6881D6A
    function Initialize()
    {
...
//Validate Method @128-987C8320
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        $Where = "";
        if(strlen($this->ListBox3->GetText()) && !preg_match ("/^[a-zA-Z]{2,2}[0-9]{6,6}$/", $this->ListBox3->GetText())) {
            $this->ListBox3->Errors->addError($CCSLocales->GetText("CCS_MaskValidation", "ListBox3"));
        }
...
//CheckErrors Method @128-633A9527
...
//Operation Method @128-4F4E49FD
    function Operation()
    {
        ...
        $Redirect = "doc_Pre_Numbering.php";
...
//UpdateRow Method @128-8C924017
    function UpdateRow()
    {
        ...
        $this->DataSource->batch_no->SetValue($this->batch_no->GetValue(true));
...
//DataSource Variables @128-DF0F7B99
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @128-AC0BFE8B
    function clsmfi_docsDataSource(& $Parent)
    {
        ...
        $this->UpdateFields["pre_numbered_at"] = array("Name" => "pre_numbered_at", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["mfi_doc_id"] = array("Name" => "mfi_doc_id", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["mfi_doc_region"] = array("Name" => "mfi_doc_region", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["batch_code"] = array("Name" => "batch_code", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
    }
//End DataSourceClass_Initialize Event

//Prepare Method @128-328EC879
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->Criterion[2] = "( mfi_doc_status='TAGGED' )";
...
//SetValues Method @128-879C0805
    function SetValues()
    {
        $this->mfi_doc_tagged_by->SetDBValue($this->f("numbered_by"));
        ...
        $this->mfi_doc_tagged_at->SetDBValue($this->f("numbered_at"));
        $this->mfi_doc_region->SetDBValue($this->f("mfi_doc_region"));
        $this->ListBox3->SetDBValue($this->f("mfi_doc_pre_number_code"));
...
//Update Method @128-21987ACB
    function Update()
    {
        ...
        $Where = "";
        $this->cp["mfi_doc_pre_number_code"] = new clsSQLParameter("ctrlListBox3", ccsText, "", "", $this->ListBox3->GetValue(true), NULL, false, $this->ErrorBlock);
        $this->cp["mfi_doc_status"] = new clsSQLParameter("expr163", ccsText, "", "", "PRENUMBERED", NULL, false, $this->ErrorBlock);
        $this->cp["pre_numbered_by"] = new clsSQLParameter("ctrlmfi_doc_tagged_by", ccsText, "", "", $this->mfi_doc_tagged_by->GetValue(true), NULL, false, $this->ErrorBlock);
        ...
        $this->cp["pre_numbered_at"] = new clsSQLParameter("ctrlmfi_doc_tagged_at", ccsText, "", "", $this->mfi_doc_tagged_at->GetValue(true), NULL, false, $this->ErrorBlock);
        $this->cp["mfi_doc_id"] = new clsSQLParameter("ctrlmfi_doc_id", ccsText, "", "", $this->mfi_doc_id->GetValue(true), NULL, false, $this->ErrorBlock);
        ...
        $this->cp["batch_code"] = new clsSQLParameter("ctrlbatch_no", ccsText, "", "", $this->batch_no->GetValue(true), NULL, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["mfi_doc_pre_number_code"]->GetValue()) and !strlen($this->cp["mfi_doc_pre_number_code"]->GetText()) and !is_bool($this->cp["mfi_doc_pre_number_code"]->GetValue())) 
            $this->cp["mfi_doc_pre_number_code"]->SetValue($this->ListBox3->GetValue(true));
        if (!is_null($this->cp["mfi_doc_status"]->GetValue()) and !strlen($this->cp["mfi_doc_status"]->GetText()) and !is_bool($this->cp["mfi_doc_status"]->GetValue())) 
            $this->cp["mfi_doc_status"]->SetValue("PRENUMBERED");
        if (!is_null($this->cp["pre_numbered_by"]->GetValue()) and !strlen($this->cp["pre_numbered_by"]->GetText()) and !is_bool($this->cp["pre_numbered_by"]->GetValue())) 
            $this->cp["pre_numbered_by"]->SetValue($this->mfi_doc_tagged_by->GetValue(true));
        ...
        if (!is_null($this->cp["pre_numbered_at"]->GetValue()) and !strlen($this->cp["pre_numbered_at"]->GetText()) and !is_bool($this->cp["pre_numbered_at"]->GetValue())) 
            $this->cp["pre_numbered_at"]->SetValue($this->mfi_doc_tagged_at->GetValue(true));
        ...
        if (!is_null($this->cp["batch_code"]->GetValue()) and !strlen($this->cp["batch_code"]->GetText()) and !is_bool($this->cp["batch_code"]->GetValue())) 
            $this->cp["batch_code"]->SetValue($this->batch_no->GetValue(true));
        ...
        $Where = $wp->opAND(
             ...
             $wp->Criterion[2]), 
             $wp->Criterion[3]);
        $this->UpdateFields["mfi_doc_pre_number_code"]["Value"] = $this->cp["mfi_doc_pre_number_code"]->GetDBValue(true);
        $this->UpdateFields["mfi_doc_status"]["Value"] = $this->cp["mfi_doc_status"]->GetDBValue(true);
        $this->UpdateFields["pre_numbered_by"]["Value"] = $this->cp["pre_numbered_by"]->GetDBValue(true);
        ...
        $this->UpdateFields["pre_numbered_at"]["Value"] = $this->cp["pre_numbered_at"]->GetDBValue(true);
        ...
        $this->UpdateFields["batch_code"]["Value"] = $this->cp["batch_code"]->GetDBValue(true);
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "doc_Pre_Numbering.html";
$BlockToParse = "main";
...
//Include events file @1-76E25FD7
include_once("./doc_Pre_Numbering_events.php");
