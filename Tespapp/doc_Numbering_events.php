...
$query = "select count(*)from mfi_docs where batch_code='".CCGetCookie('batch_code')."' and (mfi_doc_status='PRENUMBERED' or mfi_doc_status='NUMBERING')";
...
{
$db = new clsDBmysql_cams_v2();
$query = "select batch_code from mfi_docs where mfi_doc_status='PRENUMBERED' and batch_code not in(select batch_code from doctagging)group by batch_code limit 1";
$export  = $db->query($query);
	$row = mysql_fetch_row($export);
		if(count($row)>0)
		{
