...
//End Custom Code
	...
	$sql = "SELECT count(*) FROM mfi_docs WHERE (mfi_doc_status='NUMBERED' or mfi_doc_status='DATA ENTRY') and mfi_doc_region='".CCGetCookie("docregion")."' and batch_code='".CCGetCookie("batch_code")."'";
	$export = $db->query($sql);
    $row = mysql_fetch_row($export);
	$mfi_docs->Label1->SetValue($row[0]);
	if($row[0]==0)
	{
	$sql="update batch_details set status='COMPLETED',DATE_COMPLETED=now() where mfi_doc_region='".CCGetCookie("docregion")."' and batch_code='".CCGetCookie("batch_code")."'";
	$db->query($sql);
	}
	else
	{
	$sql="update batch_details set status='INCOMPLETE' where mfi_doc_region='".CCGetCookie("docregion")."' and batch_code='".CCGetCookie("batch_code")."'";
	$db->query($sql);
	}
...
//End mfi_docs_BeforeShow
		...
		$conn->query("select mfi_doc_region,batch_code  from batch_details where ca_alloted='".strtoupper(CCGetUserLogin())."' and (status='INCOMPLETE'||status='NOT ALLOTED')");
		$conn->next_record();
...
//Page_AfterInitialize @1-85AD24A6
function Page_AfterInitialize(& $sender)
{
    $Page_AfterInitialize = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $BatchDataEntry; //Compatibility
