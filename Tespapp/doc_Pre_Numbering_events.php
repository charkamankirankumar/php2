...
function BindEvents()
{
    ...
    $mfi_docs->pre_numbered_by->CCSEvents["BeforeShow"] = "mfi_docs_pre_numbered_by_BeforeShow";
...
//mfi_docs_pre_numbered_by_BeforeShow @242-405ADDC2
function mfi_docs_pre_numbered_by_BeforeShow(& $sender)
{
    $mfi_docs_pre_numbered_by_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $mfi_docs; //Compatibility
//End mfi_docs_pre_numbered_by_BeforeShow

//Custom Code @243-2A29BDB7
// -------------------------
    $mfi_docs->pre_numbered_by->SetValue(CCGetUserLogin());
// -------------------------
//End Custom Code

//Close mfi_docs_pre_numbered_by_BeforeShow @242-BB82D862
    return $mfi_docs_pre_numbered_by_BeforeShow;
}
//End Close mfi_docs_pre_numbered_by_BeforeShow
...
$query = "select count(*)from mfi_docs where batch_code='".CCGetCookie('batch_code')."' and (mfi_doc_status='TAGGED' or mfi_doc_status='PRENUMBERING')";
$export  = $db->query($query);
	$row = mysql_fetch_row($export);
	$mfi_docs->Label1->SetValue($row[0]);
	if($row[0]>0)
	{
	$query = "select count(*),mfi_doc_type from mfi_docs where batch_code='".CCGetCookie('batch_code')."' and mfi_doc_territory_code is not null group by mfi_doc_type";
	$export  = $db->query($query);
	$row = mysql_fetch_row($export);
	$pdng=$row[0];
			...
			while($row = mysql_fetch_row($export))
			{
				...
				{
				  case "GP":
				  $mfi_docs->ngp->SetValue($row[0]);
				  $t+=$row[0];
				  break;
				  case "CP":
				  $mfi_docs->ncp->SetValue($row[0]);
				  $t+=$row[0];
				  break;
				  case "GLE":
				  $mfi_docs->ngle->SetValue($row[0]);
				  $t+=$row[0];
				  break;
				  case "LA1":
				  $mfi_docs->nla1->SetValue($row[0]);
				  $t+=$row[0];
				  break;
				  case "LA2":
				  $mfi_docs->nla2->SetValue($row[0]);
				  $t+=$row[0];
				  break;
				  case "HV1":
				  $mfi_docs->nhv1->SetValue($row[0]);
				  $t+=$row[0];
				  break;
				  case "HV2":
				  $mfi_docs->nhv2->SetValue($row[0]);
				  $t+=$row[0];
				  break;
				  case "INVALID IMAGE":
				  $mfi_docs->nii->SetValue($row[0]);
				  $t+=$row[0];
				  break;
	 ...
	 $query = "delete from doctagging where batch_code='".CCGetCookie('batch_code')."'";
	 $export  = $db->query($query);
...
{
$db = new clsDBmysql_cams_v2();
$query = "select batch_code from mfi_docs where mfi_doc_status='TAGGED' and batch_code not in(select batch_code from doctagging)group by batch_code limit 1";
$export  = $db->query($query);
	$row = mysql_fetch_row($export);
		if(count($row)>0)
		{
